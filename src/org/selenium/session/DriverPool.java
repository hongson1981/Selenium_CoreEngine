...
public enum DriverPool {
	...
	//org.apache.logging.log4j.Logger logger = LogManager.getLogger(DriverPool.class.getName());
...
//				newDriver = new HtmlUnitDriver();
				/*
				DesiredCapabilities html = DesiredCapabilities.htmlUnit();
				//html.setCapability("version", "");
				//html.setCapability("platform", Platform.WIN10);
				try {
					newDriver = new RemoteWebDriver(new URL(SetUp.getSetup().getRemoteSlaveNode()), html);
			...
			case "CHROME":
				//C:\Users\sha6\AppData\Local\Google\Chrome\User Data
				System.setProperty("webdriver.chrome.driver", "C:\\Fitnesse\\chromedriver.exe");
				ChromeOptions op = new ChromeOptions();
				op.addArguments("user-data-dir=C:\\Users\\sha6\\ITS CITRIX\\selenium\\User Data");
			...
			case "CHROME_HEADLESS":
                ...
                options.addArguments("--disable-gpu", "--window-size=1920,1080", "--headless");
                DesiredCapabilities capheadless = DesiredCapabilities.chrome();
                capheadless.setCapability(ChromeOptions.CAPABILITY, options); 
				try {
					newDriver = new RemoteWebDriver(new URL(SetUp.getSetup().getRemoteSlaveNode()), capheadless);
					newDriver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
					newDriver.manage().window().maximize();
			...
			case "IE":
		        ...
		        try {
					Runtime.getRuntime().exec(
							"cmd /c start cmd.exe /K \"C:/ITS/Selenium/IEDriverServer.exe");
